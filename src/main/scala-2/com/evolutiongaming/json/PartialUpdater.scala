...
/**
 * The trait can apply json value to specified object and it do it partially only if corresponding
 * fields is defined in json.
 *
 * So you could have a
 * {{{
 ...
 *   - application of {{{{ "a": "x" }}}} will change only {{{entity.a}}} to {{{"x"}}}.
 *     {{{entity.b}}} will stay unchanged
 *   - application of {{{{ "b": "x" }}}} will change only {{{entity.b}}} to {{{Some("x")}}}.
 *     {{{entity.a}}} will stay unchanged
 *   - application of {{{{ "b": null }}}} will change only {{{entity.b}}} to {{{None}}}.
 *     {{{entity.a}}} will stay unchanged
 *
 * Dealing with partial updaters is as easy as it is in case of Play formats. The only thing you
 * have to do is declare
 * {{{
 ...
 * }}}
 *
 * If you use embedded entities, you have to make sure that all needed [[play.api.libs.json.Reads]]
 * are accessible in implicit scope.
 *
  ...
  def updaterImpl[T: c.WeakTypeTag](c: blackbox.Context): c.Expr[PartialUpdater[T]] = {
    ...
    val fields = MacroUtil.fieldMap(c)(tpe) map { case (n, t) =>
      ...
      val expr = (isOption, resultingType) match {
        ...
        case (false, ValueClass(ft: Type, i: Type)) =>
          q"val $n: $t = u.opt[$i]($path) map ${ ft.typeSymbol.companion }.apply getOrElse entity.$n"
        case (true, ValueClass(ft: Type, i: Type)) =>
          q"val $n: $t = u.optOpt[$i]($path) map {_ map ${ ft.typeSymbol.companion }.apply} getOrElse entity.$n"
        case (false, CaseClass(ft: Type)) =>
          q"val $n: $t = u.reader($path) map {x => implicitly[PartialUpdater[$ft]].apply(entity.$n, x)} getOrElse entity.$n"
