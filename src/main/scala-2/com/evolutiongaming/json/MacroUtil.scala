...
/**
 * The only purpose of this class is to provide the ability to extract Map[String, Class[_]] of case
 * class fields which are not marked by @skip annotation.
 *
    ...
    object UpdatableField {
      def unapply(s: TermSymbol): Option[(TermName, Type)] = {
        ...
        if (
          ...
          && s.isCaseAccessor
          && !shouldSkip(name)
        ) Some((TermName(name), s.typeSignature))
        else None
      }
    }
